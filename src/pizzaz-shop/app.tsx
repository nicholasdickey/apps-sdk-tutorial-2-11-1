...
export type PizzazShopAppProps = {
...
export function App({
  defaultCartItems: providedDefaultCartItems,
}: PizzazShopAppProps = {}) {
  ...
  const baseCartItems = useMemo(() => {
    if (Array.isArray(providedDefaultCartItems)) {
      return providedDefaultCartItems.map((item) => cloneCartItem(item));
    }
    return createDefaultCartItems();
  }, [providedDefaultCartItems]);
  const computeDefaultWidgetState = useCallback(
    (): PizzazCartWidgetState => ({
      state: null,
      cartItems: baseCartItems.map((item) => cloneCartItem(item)),
      selectedCartItemId: null,
    }),
    [baseCartItems]
  );
  const [widgetState, setWidgetState] = useWidgetState<PizzazCartWidgetState>(
    computeDefaultWidgetState
  );
  ...
  const mergeWithDefaultItems = useCallback(
    (items: CartItem[]): CartItem[] => {
      ...
      const merged = items.map((item) => {
        const defaultItem = baseCartItems.find(
      ...
      baseCartItems.forEach((defaultItem) => {
  ...
  const resolvedCartItems = useMemo(() => {
    ...
    return mergeWithDefaultItems(baseCartItems);
  }, [
    baseCartItems,
  ...
  const updateWidgetState = useCallback(
    ...
    [computeDefaultWidgetState, setWidgetState]
  );
